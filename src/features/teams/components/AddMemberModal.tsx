...
interface Props {
  ...
  onClose: () => void;
  teamID: string;
  onAdd: (teamID: string, userID: string, role: string) => void;
}

// Define this outside your component at the top of the file!
const TEAM_ROLES = [
...
const colors = ['bg-red-500', 'bg-blue-500', 'bg-purple-600', 'bg-green-500', 'bg-amber-500'];




export default function AddMemberModal({ isOpen, onClose, teamID, onAdd }: Props) {
  ...
  const fetchUsers = useCallback(async()=> {
    try{
      ...
      if(data.success){
        const dropDownUserData: User[] = data.data.map((d: any) => {
          return {
            userID: d.userID,
            name: d.name,
          }
        });
        setDropDownUsers(dropDownUserData);
      } else {
  ...
  const handleCreate = () => {
    ...
    console.log(selectedUserId, teamID, selectedRole);
    if (selectedUserId && teamID && selectedRole) {
      onAdd(teamID, selectedUserId, selectedRole);
      setSelectedUserId('');
      onClose();
    }
  };
  ...
  const selectedUser = dropDownUsers.find(u => u.userID === selectedUserId);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md animate-in zoom-in-95 duration-200 border border-zinc-200 flex flex-col">
            ...
            {isUserDropDownOpen && (
              <div className="absolute top-full left-0 mt-1.5 w-full max-h-48 overflow-y-auto bg-white border border-zinc-200 shadow-xl rounded-md z-50 animate-in fade-in zoom-in-95 duration-100 py-1 custom-scrollbar">
                {dropDownUsers.map(user => (
                  <button
                    key={user.userID}
                    onClick={() => { setSelectedUserId(user.userID); setIsUserDropDownOpen(false); }}
                    className="w-full flex items-center justify-between px-4 py-2 hover:bg-zinc-50 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold ${colors[Math.floor(Math.random() * colors.length)]}`}>
                        {user.name.charAt(0)}
                      </div>
                      <span className="text-[14px] font-medium text-zinc-800">{user.name}</span>
                    </div>
                    {selectedUserId === user.userID && <Check className="w-4 h-4 text-blue-600" />}
                  </button>
                ))}
              </div>
            )}
        ...
        <div className="px-8 pb-8">
          <label className="block text-sm font-semibold text-zinc-700 mb-2">Select Role</label>
          ...
          <div className="relative" ref={roleDropdownRef}>
            <button 
              ...
              className="w-full flex items-center justify-between bg-white border-2 border-zinc-200 rounded-md px-4 py-2.5 text-[15px] outline-none focus:ring-0 focus:border-blue-500 transition-colors text-zinc-900 font-medium"
            >
              {/* Find the selected role label or fallback */}
              <span>{TEAM_ROLES.find(r => r.value === selectedRole)?.label}</span>
              <ChevronDown className="w-4 h-4 text-zinc-400" />
            </button>
              ...
              <div className="absolute top-full left-0 mt-1.5 w-full max-h-48 overflow-y-auto bg-white border border-zinc-200 shadow-xl rounded-md z-50 animate-in fade-in zoom-in-95 duration-100 py-1 custom-scrollbar">
                {TEAM_ROLES.map(role => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-4 py-2 hover:bg-zinc-50 transition-colors"
                  >
                   <span className="text-[14px] font-medium text-zinc-800">{role.label}</span>
                    {selectedRole === role.value && <Check className="w-4 h-4 text-blue-600" />}
                  </button>
                ))}
              </div>
            )}
