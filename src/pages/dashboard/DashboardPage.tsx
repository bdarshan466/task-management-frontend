...
export default function DashboardPage() {
  ...
  const colors = ['bg-red-500 text-white', 'bg-blue-500 text-white', 'bg-purple-600 text-white', 'bg-green-500 text-white', 'bg-amber-500 text-white'];
...
useEffect(() => {
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setIsLoading(false); // ✅ only after BOTH APIs complete
    }
  };
              ...
              <div className="px-8 pt-6 pb-4">
                <div className="text-sm text-zinc-500 font-medium mb-3 flex items-center gap-2">
                  <div className="relative inline-block w-[240px]" ref={dropdownRef}>
                    ...
                    >
                      <span>{selectedTeam ? teamsData.find(t => t.teamID === selectedTeam)?.name + ' Team' : 'All Teams (Global Board)'}</span>
                  ...
                  <div className="flex items-center -space-x-2">
                    {usersData.map((member) => (
                      <button
                        ...
                        className={`relative w-8 h-8 rounded-full flex items-center justify-center border-2 border-background transition-transform hover:-translate-y-1 hover:z-10 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary ${colors[Math.floor(Math.random() * colors.length)]
